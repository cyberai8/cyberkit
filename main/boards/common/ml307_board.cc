...
Ml307Board::Ml307Board(gpio_num_t tx_pin, gpio_num_t rx_pin, gpio_num_t dtr_pin, uart_port_t uart_num)
...
void Ml307Board::StartNetwork() {
    ...
    while (true) {
        modem_ = AtModem::Detect(tx_pin_, rx_pin_, dtr_pin_, 921600, -1, uart_num_);
        if (modem_ != nullptr) {
            break;
        }
