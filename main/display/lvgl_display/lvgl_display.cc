...
void LvglDisplay::ShowQRCode(const char* url, const char* device_name) {
    ESP_LOGW(TAG, "ShowQRCode (LvglDisplay): %s, device: %s", url, device_name ? device_name : "null");
}

void LvglDisplay::HideQRCode() {
    ESP_LOGW(TAG, "HideQRCode (LvglDisplay)");
}
