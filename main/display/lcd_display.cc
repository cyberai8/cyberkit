...
void LcdDisplay::ShowQRCode(const char* url, const char* device_name) {
    ...
    if (qr_container_ != nullptr) {
        lv_obj_delete(qr_container_);
        qr_container_ = nullptr;
    }

    // Create a container for the QR code to ensure it's centered
    qr_container_ = lv_obj_create(content_);
    lv_obj_set_size(qr_container_, LV_HOR_RES, LV_SIZE_CONTENT);
    ...
    lv_obj_set_style_pad_all(qr_container_, 10, 0);
    ...
    // Set custom attribute to mark bubble type (so it can be themed if needed)
    lv_obj_set_user_data(qr_container_, (void*)"system");

    // Scroll to the QR code
    lv_obj_scroll_to_view_recursive(qr_container_, LV_ANIM_ON);
}
