...
public:
    ...
    std::string GetCheckVersionUrl();
     ...
     // 新增方法：连接到指定的升级服务器进行升级
    bool CheckUpgradeFromServer(const std::string& upgrade_server_url);
    bool StartUpgradeFromServer(const std::string& upgrade_server_url,
                                std::function<void(int progress, size_t speed)> callback,
                                bool skip_check = false);
    void NotifyUpgradeComplete(bool success);
    void NotifyFirmwareDeliveryStart();
    void NotifyFirmwareDeliveryComplete(bool success, size_t firmware_size);
    // 获取升级服务器URL
    std::string GetUpgradeServerUrl();
