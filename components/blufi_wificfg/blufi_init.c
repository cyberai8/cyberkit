...
// 声明外部定义的 GAP 回调函数
#ifdef CONFIG_BT_BLUEDROID_ENABLED
extern void esp_blufi_gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param);
#endif
...
esp_err_t esp_blufi_host_init(void)
{
    ...
    // 设置蓝牙设备名称，这样在广播时才会被手机扫描到
    if (g_device_name) {
        esp_ble_gap_set_device_name(g_device_name);
    }
    
    BLUFI_INFO("BD ADDR: "ESP_BD_ADDR_STR", Name: %s\n", ESP_BD_ADDR_HEX(esp_bt_dev_get_address()), g_device_name);
