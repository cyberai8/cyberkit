...
std::unique_ptr<AtModem> AtModem::Detect(gpio_num_t tx_pin, gpio_num_t rx_pin, gpio_num_t dtr_pin,
                                         int baud_rate, int timeout_ms, uart_port_t uart_num) {
    // 调用带 RI pin 的版本，RI pin 默认为 GPIO_NUM_NC
    return Detect(tx_pin, rx_pin, dtr_pin, GPIO_NUM_NC, baud_rate, timeout_ms, uart_num);
}

std::unique_ptr<AtModem> AtModem::Detect(gpio_num_t tx_pin, gpio_num_t rx_pin, gpio_num_t dtr_pin,
                                         gpio_num_t ri_pin, int baud_rate, int timeout_ms, uart_port_t uart_num) {
    // 创建AtUart进行检测
