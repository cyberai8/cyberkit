...
void HttpClient::Close() {
    ...
    if (tcp_) {
        // The receive task can still observe the socket closing and run its
        // disconnect callback. Clear callbacks first so it cannot call back
        // into this HttpClient after or during destruction.
        tcp_->OnStream(nullptr);
        tcp_->OnDisconnected(nullptr);
        tcp_->Disconnect();
    }
...
void HttpClient::OnTcpDisconnected() {
    if (!mutex_.try_lock()) {
        ...
        ESP_LOGW(TAG, "TCP disconnected while HTTP parser is busy");
        return;
    }
