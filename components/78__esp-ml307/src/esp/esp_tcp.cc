...
EspTcp::~EspTcp() {
    ...
    if (receive_task_handle_ != nullptr && event_group_ != nullptr &&
        ...
        if (!(bits & ESP_TCP_EVENT_RECEIVE_TASK_EXIT)) {
            ESP_LOGE(TAG, "Failed to wait for receive task exit in destructor");
        }
    }
...
void EspTcp::DoDisconnect(bool wait_for_task) {
    ...
    // 断开连接时触发断开回调
    if (disconnect_callback_ && !disconnect_callback_running_) {
        disconnect_callback_running_ = true;
        disconnect_callback_();
        disconnect_callback_running_ = false;
    }
}
