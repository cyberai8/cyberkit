...
static esp_err_t panel_st77916_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end, const void *color_data)
{
    ...
    // Propagate the color-transfer error.  The SPI panel IO does not invoke
    // the completion callback when queueing fails; hiding this error makes
    // synchronous renderers wait forever for a callback that cannot arrive.
    ESP_RETURN_ON_ERROR(tx_color(st77916, io, LCD_CMD_RAMWR, color_data, len),
